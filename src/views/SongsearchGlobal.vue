          ...
          <div class="container">
                  ...
                  >
                    <ul class="search-hidtory-ul">
                      <li v-for="(artisthis) in hisToryArray" :key="artisthis.id">
                        <label @click="searchFromList(artisthis.fileTitle)">
                          <img
                            v-if="artisthis.musicImage == null"
                            src="assets/images/fan-profile-picture.png"
                            alt
                          >
                          <img v-else :src="allpath.user_uploaded_musics+artisthis.musicImage">
                          <span>
                            <a href="javascript:void(0)">{{artisthis.fileTitle}}</a>
                          </span>
                        </label>
                  ...
                  <div class="search_result-box" :class="{'show': AutoComplete.length>0 }">
                    <ul class="search-hidtory-ul">
                      <li
                        ...
                        :key="artist.id"
                        @click="searchFromList(artist.fileTitle)"
                      >
                        <img
                        ...
                        <span>
                          <a href="javascript:void(0)">{{artist.fileTitle}}</a>
                        </span>
            ...
            <!-- <ul class="main-search-menu">
             ...
             <i class="icon-microphone"></i> Artists
              ...
              <a href="/songglobalsearch"><i class="icon-music-player">
                 </i> Songs</a>
            ...
            <div class="row justify-content-between align-items-center seacrhsubdivcat">
              <ul class="serach-tab-menu">
                <li :class="{ active : sortByValue === '' }">
                  <a href="javascript:;" id="popular" v-on:click="artistSongSort('')">
                    <i class="icon-fame"></i> Popular
                  </a>
                </li>
                <li :class="{ active : sortByValue === 'new' }">
                  <a href="javascript:;" id="new" v-on:click="artistSongSort('new')">
                    <i class="icon-new-document"></i> New
                  </a>
                </li>
                <li  :class="{ active : sortByValue === 'random' }">
                  <a href="javascript:;" id="random" v-on:click="artistSongSort('random')">
                    <i class="icon-suffle"></i> Random
                  </a>
                </li>
                <li :class="{ active : sortByValue === 'votersPick'}">
                  <a href="javascript:;" id="votPic" v-on:click="artistSongSort('votersPick')">
            ...
            <div class="row align-items-center">
              <div v-for="(artist,index) in Artist" :key="index" class="albumeach">
                <div class="albumeachimg">
                  <img v-if="artist.musicImage == null" src="assets/images/default_songs.png">
                  <img v-else :src="allpath.user_uploaded_musics+artist.musicImage">
                </div>
...
export default {
  ...
  created() {
    this.hisToryArray = JSON.parse(localStorage.getItem("hisToryArrayForsong"));
    if (this.hisToryArray == null) {
      this.hisToryArray = [];
    }
    console.log("here", this.hisToryArray);
    this.initialSearchList();
  },
  mounted() {
    var that = this;
